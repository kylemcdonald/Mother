...
void MarchingTetrahedrons::setCenters(vector<ofVec3f>& centers) {
...
void MarchingTetrahedrons::buildCells() {
	...
	cout << "total cells worth evaluating: " << totalCells << endl;
	
	unsigned long stop = ofGetSystemTimeMicros();
	cout << "cells took " << (stop - start) / 1000. << " ms" << endl;
...
void MarchingTetrahedrons::buildMesh() {
	...
	unsigned long stop = ofGetSystemTimeMicros();
	cout << "meshing took " << (stop - start) / 1000. << " ms" << endl;
	cout << "total vertices: " << mesh.getNumVertices() << endl;
	...
	cout << "full cells: " << (total - empty) << endl;
	cout << "empty cells: " << empty << " (" << (100 * empty) / total << "%)" << endl;
}
...
void MarchingTetrahedrons::draw() {
	...
	for(int i = 0; i < centers.size(); i++) {
		ofPushMatrix();
		ofTranslate(centers[i].x, centers[i].y, centers[i].z);
		ofSphere(minRadius);
		ofSphere(maxRadius);
		ofPopMatrix();
	}
...
void MarchingTetrahedrons::drawCubes() {
 int offsets[] = {
		getIndex(0, 0, 0),
		getIndex(1, 0, 0),
		getIndex(0, 1, 0),
	...
	int offset = 0;
	ofRotateX(180);
	float scale = 8 * (512 / 16);
	ofScale(scale, scale, scale);
	ofTranslate(-(offset % 16), -(offset / 16));
	for(int i = 0; i < 256; i++) {
		...
		for(int j = 0; j < samples.size(); j++) {
			pair<int, int>& curSample = samples[j];
			ofVec3f from = getVertex(offsets[curSample.first]);
			ofVec3f to = getVertex(offsets[curSample.second]);
			ofVec3f center = (from + to) / 2;
		...
		ofPushMatrix();
		ofScale(resolution, resolution, resolution);
		...
		for(int j = 0; j < 8; j++) {
			if(getBit(state, j)) {
				ofVec3f cur = getVertex(offsets[j]);
				points.addVertex(cur);
				ofDrawBitmapString(ofToString(j), cur);
			}
		}
		ofMesh tetras;
		tetras.setMode(OF_PRIMITIVE_TRIANGLES);
		for(int j = 0; j < 6; j++) {
			tetras.addVertex(getVertex(offsets[tetrahedronIndices[4*j + 0]]));
			tetras.addVertex(getVertex(offsets[tetrahedronIndices[4*j + 1]]));
			tetras.addVertex(getVertex(offsets[tetrahedronIndices[4*j + 2]]));
			
			tetras.addVertex(getVertex(offsets[tetrahedronIndices[4*j + 1]]));
			tetras.addVertex(getVertex(offsets[tetrahedronIndices[4*j + 2]]));
			tetras.addVertex(getVertex(offsets[tetrahedronIndices[4*j + 3]]));
			
			tetras.addVertex(getVertex(offsets[tetrahedronIndices[4*j + 0]]));
			tetras.addVertex(getVertex(offsets[tetrahedronIndices[4*j + 2]]));
			tetras.addVertex(getVertex(offsets[tetrahedronIndices[4*j + 3]]));
		}
		buildNormalsSingle(tetras);
		ofSetColor(ofColor::red);
		points.draw();
		ofSetColor(ofColor::gray);
		tetras.drawWireframe();
		ofPopMatrix();
